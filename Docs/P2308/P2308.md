...
- 进程： 行为(fork,wait)，状态(ready to run, running,waiting),属性(page table,stack)，仅支持单线程
- 页表: 虚拟地址空间
...
|:arrow_double_up:| kernel/main.c |初始化kernel,开启第一个进程 用户进程init，Supervisor Mode|
|:arrow_double_up:| kernel/start.c |设定 sp ，开启时钟中断 Machine Mode|
|:arrow_double_up:| kernel/entry.S |set up a stack so that xv6 can run C code  Machine Mode 虚拟地址不可用|
|:arrow_double_up:| boot_loader | 把kernel.bin 加载到物理地址 0x80000000|
|:arrow_double_up:| 初始化 boot_loader|| 
