...
- openGauss 是一款支持 SQL2003 标准语法，支持**主备部署**的高可用关系型数据库。
...
:white_check_mark: 轻量版

![lite-version](lite-version.png)
...
https://docs-opengauss.osinfra.cn/zh/docs/latest-lite/docs/CompilationGuide/%E6%90%AD%E5%BB%BA%E7%BC%96%E8%AF%91%E7%8E%AF%E5%A2%83.html
...
- RISC-V通用云开发空间
  https://cloud.sophgo.com/developer/cloudSpace/spaceDetail/16
    ...
    | CPU  | 操作系统                                 |内存 | 磁盘    |
    | -------- | ----------------------------------- | ------------------- | ----------- |
    | SG2042 /16核    | openKylin                      |  50GB             | 256GB  |
     
    注意： 云开发空间是分时复用的，用户只能在可用排期内才可登录使用

- EPCC RISC-V testbed
  https://riscv.epcc.ed.ac.uk/documentation/getting_started/

- Pioneer Box :white_check_mark:
...
```
- 手动配置 gcc 7.3 尝试在 build-tmp 文件夹完成编译
...
python3 build.py -m all -f openssl-1.1.1g.tar.gz -t "comm|llt"
```
![build-openssl](build-openssl.png)
...
## 总结
对比 VirtualBox 与 SG2042(PioneerBox) 环境差异，以及取得的进展情况分析

### 环境差异

| 差异点  | SG2042 | VirtualBox |
| -------- | ------------------- | ----------- |
| CPU    |  SG2042 / 64 核                      |  x86_64 / 4 核             | 
...
- 安装配置 openGauss-server :x: :white_check_mark:
- 配置 TPCC 性能测试 BenchmarkSQL-5.0
- BenchmarkSQL-5.0 稳定性测试和性能测试结果 :x:

### 在 SG2042(PioneerBox) 移植情况分析

- 从源码编译 gcc7.3/gcc10.3 :x:
- 基于 native gcc 编译 binaryLibs 中的模块 :white_check_mark:
